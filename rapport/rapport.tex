...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../outputs/predictions_vs_reel_zoom.png}
    \caption{Prédictions vs valeurs réelles - Zoom sur les petites valeurs}
    \label{fig:pred_zoom}
\end{figure}

La Figure~\ref{fig:pred_zoom} permet d'observer la qualité des prédictions pour les 
communes avec peu d'accidents, où la majorité des données se concentre. On constate 
que les modèles sont globalement bien calibrés pour ces valeurs.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../outputs/predictions_vs_reel.png}
    \caption{Prédictions vs valeurs réelles - Vue globale (incluant les outliers)}
    \label{fig:pred}
\end{figure}

La Figure~\ref{fig:pred} montre la vue d'ensemble incluant les communes avec 
beaucoup d'accidents (outliers comme Paris), confirmant la robustesse des modèles 
sur toute la plage de valeurs.

...
\author{\centering Nicolas Huyghe \quad David Chhoa \quad Jérémie Masnou\\[0.5cm]
...
\begin{abstract}
Ce projet fusionne trois jeux de données ouverts (accidents vélo, aménagements cyclables, comptages) pour analyser la sécurité routière des cyclistes en Île-de-France. Nous avons abordé deux problématiques : la classification du niveau de risque communal et la régression sur le nombre d'accidents. L'analyse des résultats de classification s'est révélée non concluante en raison de biais méthodologiques. Nous avons donc enrichi notre approche en intégrant un quatrième jeu de données (population INSEE) afin de prédire des taux de risque normalisés, définis de deux manières : le taux d'accidents par kilomètre d'aménagement cyclable et le taux d'accidents pour 10~000 habitants.
...
\textbf{Élimination du biais de taille.} En normalisant par la population ou la longueur d'aménagement, le modèle ne peut plus identifier simplement les grandes communes. Paris n'a plus un poids disproportionné.
...
L'\textbf{approche par taux de risque normalisés} (par habitant ou par kilomètre d'aménagement) offre une méthodologie plus rigoureuse. Les performances plus modestes (R² de 20-30\%) reflètent la complexité réelle du phénomène et l'insuffisance des variables disponibles.
